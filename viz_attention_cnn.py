...
cfg.model.vit.cnn_pool_config['keep_num'] = 196
...
attn_map_np = attn_map.detach().cpu().numpy().squeeze()  # [14, 14]
att_112 = np.kron(attn_map_np, np.ones((8, 8), dtype=int))  # duplicate each attention value to all pixels in patch
...
if cfg.model.vit.cnn_pool_config is not None:
    ...
    if keep_indexes is not None:
        ...
        # plt.imshow(att_112, cmap='jet', alpha=.4)
plt.title(f"Keep {cfg.model.vit.cnn_pool_config['keep_num']} Patches")
